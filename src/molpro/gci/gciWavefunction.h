...
#include "molpro/gci/gciDeterminant.h"
#include "molpro/gci/gciFile.h"
#include "molpro/gci/gciOrbitals.h"
...
public:
  ...
  void diagonalOperator(const molpro::Operator &op); ///< set this object to the diagonal elements of the hamiltonian
   ...
   */
  void operatorOnWavefunction(const molpro::Operator &h, const Wavefunction &w, bool parallel_stringset = false);
   ...
   * \return
   */
  molpro::Operator density(int rank = 2, bool uhf = false, bool hermitian = true, const Wavefunction *bra = nullptr,
                           std::string description = "", bool parallel_stringset = false) const;
  ...
  void axpy(double a, const std::shared_ptr<Wavefunction> &x) { axpy(a, *x); }
   ...
   */
  std::vector<std::size_t> histogram(const std::vector<double> &edges, bool parallel = true, std::size_t start = 0,
  ...
  double update(const Wavefunction &diagonalH, double &eTruncated,
                const double dEmax = (double)0); ///< form a perturbation-theory update, and return the predicted energy
                                                 ///< change. eTruncated is the energy change lost by truncation
  ...
  double dot(const std::shared_ptr<Wavefunction> other) const { return dot(*other); }

  double dot(const std::unique_ptr<Wavefunction> other) const { return dot(*other); }
   ...
   */
  void divide(const Wavefunction *a, const Wavefunction *b, double shift = 0, bool append = false,
              bool negative = false);
  void zero();
