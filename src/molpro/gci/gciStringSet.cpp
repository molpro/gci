...
StringSet::StringSet() : molpro::vector<String>() {
  //      cout <<parallel_rank<<"StringSet default constructor"<<std::endl;
}

StringSet::StringSet(const String &prototype, bool all, int sym) : molpro::vector<String>() {
  //      cout <<parallel_rank<<"StringSet prototype constructor "<<all<<", sym="<<sym<<std::endl;
  // copy prototype
  proto = prototype;
  //  cout << "prototype="<<proto<<std::endl;
  symmetry = sym;
...
StringSet::StringSet(const StringSet &referenceSpace, int annihilations, int creations, int sym, bool parallel)
  ...
  //  cout << "StringSet constructor from referenceSpace size()="<<size()<<", rank="<<parallel_rank<<",
...
StringSet::StringSet(const std::vector<StringSet> &referenceSpaces, int annihilations, int creations, int sym,
                     bool parallel)
    : molpro::vector<String>() {
  //  cout << "StringSet constructor from referenceSpaces referenceSpaces:" << std::endl;
  //  for (const auto &ss: referenceSpaces) for (const auto &s: ss) cout << "referenceSpace String " << s << std::endl;
  addByOperators(referenceSpaces, annihilations, creations, sym, parallel);
  //  cout << "StringSet constructor from referenceSpaces size()=" << size() << ", rank=" << parallel_rank << ",
  //  parallel="
  //       << parallel << std::endl;
}
...
void StringSet::addByOperators(const std::vector<StringSet> &referenceSpaces, int annihilations, int creations, int sym,
  ...
  if (parallel) {
    ...
    //  cout << "parallel_rank="<<parallel_rank<<", ntask="<<ntask<<std::endl;
    //        DivideTasks(ntask);
  }
  //  cout << "about to call addByOperators " << annihilations << creations << sym << std::endl;
  for (const auto &referenceSpace : referenceSpaces) {
    //    cout << "Stringset " << *referenceSpace << std::endl;
    addByOperators(referenceSpace, annihilations, creations, sym, parallel);
  }
//  cout << "size="<<size()<<std::endl;
  ...
  if (parallel) {
    ...
    //  cout << parallel_rank<<"serialised "<<serialised.size()<<" bytes from "<<size()<<" String objects="<<std::endl;
    //  cout << parallel_rank<<"addressMap.size()"<< addressMap.size()<<std::endl;
    //  for (size_t i=0; i<parallel_size; i++){
    //  MPI_Barrier(MPI_COMM_COMPUTE);
    //  if (i!=parallel_rank)continue;
    //  cout << "Barrier crossed parallel_size="<<parallel_size<<", parallel_rank="<<parallel_rank<<std::endl;
    //  cout << "end of addByOperators creation phase, size()="<<size()<<std::endl;
    //  cout << "unmerged addressMap rank="<<parallel_rank<<", size="<<addressMap.size()<<std::endl;
    //  for (std::map<size_t,size_t> ::const_iterator a=addressMap.begin(); a!=addressMap.end(); a++)
    //    cout <<parallel_rank<<": "<<(*a).first<<","<<(*a).second<<std::endl;
    //  }
    ...
    for (initial_symmetry = 0; initial_symmetry < 8; initial_symmetry++)
      if (!referenceSpaces[initial_symmetry].empty())
        break;
    if (initial_symmetry > 7) {
      cout << "Something has gone wrong with discovering reference space in gci::StringSet::addByOperators"
           << std::endl;
      exit(1);
    }
    ...
    if (parallel_rank > 0) {
      ...
      //    cout << "slave sends len="<<len<<std::endl;
      if (len > 0) {
      ...
      //        std::cout << "slave after receving broadcast len "<<len<<", bytestreamsize="<<bytestreamsize<<std::endl;
      //        std::cout.flush();
      serialised.resize(static_cast<unsigned long>(len * bytestreamsize));
      clear();
      addressMap.clear();
      //    cout << "slave ready to bcast"<<std::endl;
      ...
      //    cout <<"master size()="<<size()<<", bytestreamsize="<<serialised.size()/(size() ? size() : 1) <<",
      //    serialised.size()"<<serialised.size()<<std::endl;
      for (int iproc = 1; iproc < parallel_size; iproc++) {
        int len;
        MPI_Status status;
        MPI_Recv(&len, (int)1, MPI_INT, iproc, 0, mpi_comm_compute, &status);
        //    cout << "master receives len="<<len<<std::endl;
        if (len > 0) {
          MPI_Recv(&bytestreamsize, (int)1, MPI_INT, iproc, 1, mpi_comm_compute, &status);
          //      cout <<"received len="<<len<<", bytestreamsize="<<bytestreamsize<<std::endl;
          ...
          for (size_t k = 0; k < (size_t)len; k++) {
            std::vector<char> s(static_cast<unsigned long>(bytestreamsize));
            memcpy(&s[0], &serialised[k * bytestreamsize], static_cast<size_t>(bytestreamsize));
            //        cout << "master construct string"<<std::endl;
            String ss(s, &refString);
            //        cout <<"master before insert"<<std::endl;
            insert(ss);
            //        cout <<"master after insert"<<std::endl;
          }
          serialised.clear();
          for (const auto &s : *this) {
            std::vector<char> serialised1 = s.serialise();
      ...
      auto len = (int)size();
      //        cout <<"master after serialising global list"<<std::endl;
      //    cout << "master ready to bcast len="<<len<<", bytestreamsize="<<bytestreamsize<<std::endl;
      bytestreamsize = serialised.size() / std::max((int)size(), 1);
      MPI_Bcast(&len, (int)1, MPI_INT, 0, mpi_comm_compute);
      MPI_Bcast(&bytestreamsize, (int)1, MPI_INT, 0, mpi_comm_compute);
      // cout <<"master after broadcasting len"<<std::endl;
      //    cout << "master ready to bcast"<<std::endl;
      MPI_Bcast(&serialised[0], len * bytestreamsize, MPI_BYTE, 0, mpi_comm_compute);
      //        cout <<"master after broadcasting global list"<<std::endl;
    }
    //    cout << "Reached end of forked code rank="<<parallel_rank<<std::endl;
    //  for (size_t i=0; i<parallel_size; i++){
    //  MPI_Barrier(MPI_COMM_COMPUTE);
    //  if (i!=parallel_rank)continue;
    //  cout << "Barrier crossed parallel_size="<<parallel_size<<", parallel_rank="<<parallel_rank<<std::endl;
    //  cout << "end of addByOperators size()="<<size()<<std::endl;
    //  cout << "merged addressMap rank="<<parallel_rank<<", size="<<addressMap.size()<<std::endl;
    //  for (std::map<size_t,size_t> ::const_iterator a=addressMap.begin(); a!=addressMap.end(); a++)
    //    cout <<parallel_rank<<": "<<(*a).first<<","<<(*a).second<<std::endl;
    //  }
  }
...
void StringSet::addByOperators(const StringSet &referenceSpace, int annihilations, int creations, int sym,
  ...
  bool first = empty();
  symmetry = sym;
  //  cout << "in StringSet::addByOperators, referenceSpace=" << referenceSpace.str(5) << std::endl;
  //  cout << "referenceSpace.symmetry=" << referenceSpace.symmetry << ", nelec=" << referenceSpace.proto.nelec
  ...
  for (auto from : referenceSpace) {
    ...
    if (annihilations + creations == 1) {
      ...
      //     cout << "annihilations="<<annihilations<<", creations="<<creations<<", first="<<first<<std::endl;
        ...
        if (phasea) {
          for (int i = 0; i < (int)from.orbitalSpace->orbital_symmetries.size(); i++) {
            if (from.orbitalSpace->orbital_symmetries[i] ==
                    ((unsigned int)symexc ^ from.orbitalSpace->orbital_symmetries[j]) ||
                symexc == -1) {
              String tt = a;
  ...
  //  cout << "addByOperators parallel="<<parallel<<", parallel_rank="<<parallel_rank<<", count="<<count<<",
...
void StringSet::setupPartialWeightArray() { // set up partial weight array for addressing binomial distributions
  ...
  for (int k = 0; k < nitem; k++) {
    ...
    for (int l = k; l < nbox - nitem + k; l++)
      PartialWeightArray[k][l + 1] =
          binomial_coefficient(static_cast<unsigned long>(nbox - l - 1), static_cast<unsigned long>(nitem - k - 1)) +
  ...
  //    cout << "PartialWeightArray:"<<std::endl; for (int k=0;k<nitem;k++) { for (int l=0;l<nbox;l++) cout << "
  //    "<<PartialWeightArray[k][l]; cout <<std::endl; }
}
...
void StringSet::complete(int sym) {
  ...
  if (string.first(proto.nelec, sym)) {
    ...
    do {
      //              cout << "in StringSet::complete about to push_back " << string.str(1) <<std::endl;
      this->insert(string);
    } while (string.next(sym));
  }
  //      cout << "in StringSet::complete final list: " <<std::endl ;
  //      for (iterator s=this->begin(); s!=this->end(); s++) cout << s->str()<<std::endl;
}

// void StringSet::insert(String& s)
//{
////    std::cout <<parallel_rank<< "StringSet::insert "<<s.str()<<std::endl;std::cout.flush();
////    cout <<parallel_rank<< "addressMap has "<<addressMap.size()<<" entries; size()="<<size()<<std::endl;
//  s.keygen(PartialWeightArray);
////  cout << "s.key="<<s.key<<", s.orbitals().size()="<<s.orbitals().size()<<std::endl;
  ...
  if (!this->empty()) {
    ...
    for (auto &s : *this) {
      ...
      for (unsigned char &orbital : orbitals) {
        //                cout << "StringSet::occupationNumbers stringoffset="<<stringoffset<<" *i="<<*i<<std::endl;
